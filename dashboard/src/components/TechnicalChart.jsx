...
import { useTechnical, useInfo } from '../hooks/useMarketData'
import { Search, TrendingUp, TrendingDown, Activity, BarChart2, Loader } from 'lucide-react'
  ...
  return (
    <ResponsiveContainer width="100%" height={80}>
      <ComposedChart data={data} margin={{ top: 2, right: 0, bottom: 0, left: 0 }}>
        ...
        <Tooltip
          contentStyle={{ background: '#1a1d26', border: '1px solid #252836', borderRadius: '8px', fontSize: '10px' }}
    ...
    <ResponsiveContainer width="100%" height={60}>
      <ComposedChart data={data} margin={{ top: 2, right: 0, bottom: 0, left: 0 }}>
        ...
        <Tooltip
          ...
          labelFormatter={() => ''}
        />
        <Bar dataKey="volume" fill="#3b82f6" fillOpacity={0.5} radius={[1, 1, 0, 0]} />
...
export default function TechnicalChart() {
  ...
  const { data: info, loading: infoLoading } = useInfo(symbol)
  ...
  // Thin the data for display if too many points
    ...
    <div className="space-y-4 animate-fade-in">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Symbol (e.g. TSLA, MSFT, SPY…)"
                value={input}
                onChange={e => setInput(e.target.value)}
                className="input-dark w-60 pl-8"
              />
            </div>
          ...
          <div className="flex gap-1.5">
            {PERIODS.map(p => (
              <button
                key={p}
                onClick={() => setPeriod(p)}
                className={`text-xs px-2.5 py-1.5 rounded transition-colors ${period === p ? 'bg-accent-blue text-white' : 'bg-bg-secondary text-text-secondary hover:text-text-primary border border-border-default'}`}
              >
                {p}
              </button>
            ))}
          ...
          <div className="flex gap-3 ml-auto text-xs">
            <label className="flex items-center gap-1.5 cursor-pointer text-text-secondary">
              <input type="checkbox" checked={showSMAs} onChange={e => setShowSMAs(e.target.checked)} className="accent-blue-500" />
              SMAs
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer text-text-secondary">
              <input type="checkbox" checked={showBB} onChange={e => setShowBB(e.target.checked)} className="accent-blue-500" />
              Bollinger
            </label>
          </div>
        </div>
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <div className="flex items-center gap-3">
                ...
                {info?.name && <span className="text-text-muted text-sm">{info.name}</span>}
                {info?.sector && <span className="text-xs px-2 py-0.5 bg-accent-blue/15 text-accent-blue rounded">{info.sector}</span>}
              </div>
              {latest && (
                <div className="flex items-center gap-3 mt-1.5">
                  <span className="text-3xl font-bold mono">${latest.close?.toFixed(2)}</span>
                  {priceChange != null && (
                    <span className={`flex items-center gap-1 text-sm font-medium ${priceChange >= 0 ? 'stat-up' : 'stat-down'}`}>
            ...
            <div className="grid grid-cols-3 lg:grid-cols-6 gap-2">
              ...
              <Badge label="Div Yield" value={info?.div_yield} format={v => (v*100).toFixed(2)+'%'} up={true} />
      ...
      {techLoading && (
        <div className="card flex items-center justify-center py-16 gap-3 text-text-muted">
          ...
          <span className="text-sm">Fetching {symbol} data from yfinance…</span>
        </div>
      )}

      {techError && !techLoading && (
        <div className="card py-10 text-center">
          <p className="text-accent-red text-sm mb-1">Could not load live data for {symbol}</p>
          <p className="text-text-muted text-xs">Make sure the API server is running: <code className="mono text-accent-cyan">cd api && uvicorn main:app --reload</code></p>
        </div>
      )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              },
              {
                ...
                sub: `MACD ${latest?.macd?.toFixed(2)} / Sig ${latest?.macd_signal?.toFixed(2)}`,
                color: macdBull ? 'stat-up' : 'stat-down',
                icon: TrendingUp,
              },
              {
                label: 'vs SMA 50',
                value: latest?.sma50 ? `${latest.close > latest.sma50 ? 'ABOVE' : 'BELOW'}` : '—',
                sub: latest?.sma50 ? `SMA50 $${latest.sma50.toFixed(2)}` : '',
                color: latest?.close > latest?.sma50 ? 'stat-up' : 'stat-down',
                icon: BarChart2,
              },
              {
                label: 'vs SMA 200',
                value: latest?.sma200 ? `${latest.close > latest.sma200 ? 'ABOVE' : 'BELOW'}` : '—',
                sub: latest?.sma200 ? `SMA200 $${latest.sma200.toFixed(2)}` : '',
                color: latest?.close > latest?.sma200 ? 'stat-up' : 'stat-down',
                icon: TrendingUp,
              },
            ].map(s => (
          ...
          <div className="card">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-3 text-[10px]">
                <span className="flex items-center gap-1"><span className="w-3 h-0.5 bg-accent-blue inline-block" /> Price</span>
                {showSMAs && <>
                  <span className="flex items-center gap-1"><span className="w-3 h-0.5 bg-accent-cyan inline-block" /> SMA20</span>
                  <span className="flex items-center gap-1"><span className="w-3 h-0.5 bg-accent-amber inline-block" /> SMA50</span>
                  <span className="flex items-center gap-1"><span className="w-3 h-0.5 bg-accent-red inline-block" /> SMA200</span>
                </>}
            ...
            <ResponsiveContainer width="100%" height={280}>
              <ComposedChart data={displayData} margin={{ top: 5, right: 5, bottom: 5, left: 0 }}>
                ...
                <YAxis tick={{ fill: '#4b5563', fontSize: 10 }} axisLine={false} tickLine={false}
                  tickFormatter={v => `$${v.toFixed(0)}`} domain={['auto', 'auto']} width={50} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="card">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`text-xs font-bold mono ${rsiSignal.color}`}>
                  {rsiLatest?.toFixed(1)} — {rsiSignal.label}
                </span>
              </div>
            ...
            <div className="card">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`text-xs font-bold mono ${macdBull ? 'stat-up' : 'stat-down'}`}>
            ...
            <div className="card">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-text-muted mono">
                  {latest?.volume ? `${(latest.volume / 1e6).toFixed(2)}M` : '—'}
                </span>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 text-xs">
              {[
                ...
                <div key={item.label} className="bg-bg-secondary rounded-lg p-2.5">
                  ...
                  <p className={`mono font-semibold ${item.up === true ? 'stat-up' : item.up === false ? 'stat-down' : 'text-text-primary'}`}>
                    {item.value}
                  </p>
                </div>
              ))}
